...
def verify(apk, android, variant='development'):
    if variant not in ('development', 'photo', 'playerrestore'):
    ...
    identity = 'com.AeDeong.MonsterTamer.revival.' + ('playerrestore' if variant == 'playerrestore' else 'gameplay')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--variant', choices=['development', 'photo', 'playerrestore'], default='development')
    args = parser.parse_args()
    root = Path(__file__).resolve().parents[2]
    suffix = '-photo' if args.variant == 'photo' else ''
    stem = 'playerrestore' if args.variant == 'playerrestore' else f'gameplay{suffix}'
