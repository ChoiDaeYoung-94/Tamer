"""Read-only metadata and manifest checks for the offline DataManager journal probe APK."""
import hashlib
import json
import re
import subprocess
...
apk = root / 'Build/revival/Tamer-journal.apk'
...
for expected in ["name='com.AeDeong.MonsterTamer.revival.journal'", "versionCode='26'",
...
result = {'apk': 'Build/revival/Tamer-journal.apk', 'sha256': digest, 'bytes': apk.stat().st_size,
          'applicationId': 'com.AeDeong.MonsterTamer.revival.journal', 'versionCode': 26,
...
(root / 'Logs/revival/journal-apk-verification.json').write_text(json.dumps(result, indent=2) + '\n')
