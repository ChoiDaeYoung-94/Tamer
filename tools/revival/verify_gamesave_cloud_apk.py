"""Read-only metadata and manifest checks for the cloud eight-field DataManager game-save probe APK."""
import hashlib
import json
import re
import subprocess
...
for expected in ["name='com.AeDeong.MonsterTamer.revival.gamesavecloud'", "versionCode='26'",
...
for forbidden in ['android.permission.ACCESS_NETWORK_STATE', 'com.android.vending.BILLING', 'com.google.android.gms.permission.AD_ID',
                  'com.google.android.gms.ads.MobileAdsInitProvider']:
    if forbidden in manifest:
        raise ValueError('Forbidden merged manifest entry: ' + forbidden)
if 'android.permission.INTERNET' not in manifest:
    raise ValueError('Expected explicit cloud INTERNET permission')
...
result = {'apk': 'Build/revival/Tamer-gamesave-cloud.apk', 'sha256': digest, 'bytes': apk.stat().st_size,
          'applicationId': 'com.AeDeong.MonsterTamer.revival.gamesavecloud', 'versionCode': 26,
...
(root / 'Logs/revival/gamesave-cloud-apk-verification.json').write_text(json.dumps(result, indent=2) + '\n')
