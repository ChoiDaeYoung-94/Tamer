param(
    ...
    [ValidateSet('development', 'photo', 'agechoice', 'sessionguard')][string]$Variant = 'development'
)
...
try {
    $gameplayHarnessMethod = if ($Variant -eq 'sessionguard') { 'RevivalGameplayBuild.BuildSessionAndroid' } elseif ($Variant -eq 'agechoice') { 'RevivalGameplayBuild.BuildAgeChoiceAndroid' } elseif ($Variant -eq 'photo') { 'RevivalGameplayBuild.BuildPhotoAndroid' } else { 'RevivalGameplayBuild.BuildAndroid' }
    $gameplayHarnessCli = Join-Path $ProjectPath 'tools/.local/unity-cli/1.0.0-beta.8/unity.exe'
    $gameplayHarnessLog = Join-Path $ProjectPath $(if ($Variant -eq 'sessionguard') { 'Logs/revival/sessionguard-build.log' } elseif ($Variant -eq 'agechoice') { 'Logs/revival/agechoice-build.log' } elseif ($Variant -eq 'photo') { 'Logs/revival/gameplay-photo-build.log' } else { 'Logs/revival/gameplay-build.log' })
