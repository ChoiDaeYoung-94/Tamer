...
def main():
    ...
    with zipfile.ZipFile(aab) as archive:
        abis = sorted({p.split("/")[2] for p in archive.namelist() if p.startswith("base/lib/") and p.endswith(".so")})
    if abis != ["arm64-v8a"]:
    ...
    result = dict(sha256=digest(aab), bytes=aab.stat().st_size, applicationId="com.AeDeong.MonsterTamer.iaptest",
                  debuggable=False, targetSdk=36, abi=abis, bundletool=VERSION, dedicatedTestSignatureVerified=True,
