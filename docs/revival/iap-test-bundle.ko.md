...
### 사용자 설치 완료 후 실제 설치·기동 확인

같은 날 사용자가 설치 완료를 알린 후, 기본 사용자 영역에서 `.iaptest`의 `installed=true`, 버전 코드 26/이름 1.0.5, installer와 initiating package `com.android.vending`, ARM64 및 비디버그 플래그를 확인했다. 전달된 패키지는 minSdk 32/targetSdk 36이며 원본 AAB의 minSdk 24 기록과 구분한다. 이전 다운로드 전 오류가 현재 설치를 막지는 않지만, 해결 원인을 반영 지연 등으로 소급 단정하지 않는다.

사용자가 열어 둔 앱에서 격리 IAP 패널이 표시됨을 확인했다. 테스트 로그인 버튼 1회는 `PlayerCreationDisabled`를 반환했고 Store는 `NotInitialized`였다. 새 설치 식별자가 기존 관리자 생성 테스트 계정에 아직 연결되지 않은 상태이며, 구매·복원 버튼은 비활성이다. 새 Store 설치 식별자만 검증해 비공개 수동 입력 파일을 준비했고 브라우저로 자동 전달하지 않았다. 기존 폰 식별자 재사용이나 계정 생성 정책 확대는 하지 않았다.

실제 설치된 APK 두 개를 비공개로 읽어 Android build-tools 36.0.0의 `apksigner verify --print-certs`와 `zipalign -c -P 16 4`를 실행했다. 두 APK 모두 각 명령 exit 0이며 서명 인증서 SHA-256은 Console의 Play 앱 서명 지문과 일치했다. apksigner는 v3 추가 속성 `0xbf940529`, `0x9f06b79c`를 알 수 없다는 경고도 출력했으므로 무경고 결과는 아니다.

| 실제 전달 파일 | 바이트 | SHA-256 |
|---|---:|---|
| base.apk | 49,602,243 | `838f4a1519bf6973d5af6f6a08f5b86aed33f1b73d50cda3318abe985c138d0a` |
| split_config.arm64_v8a.apk | 37,720,664 | `88efabda78f719a893a46774556bde6651d7897fab651c89f8db9f7a48b42c41` |

이는 새 개인폰의 실제 Store 설치·기동 및 해당 전달 APK의 서명/ZIP 검사 성공이다. 폰의 4KB 페이지 환경을 16KB 실행 검증으로 대체하지 않으며 별도 strict RELRO 실패는 유지한다. 같은 checkout과 고정 도구 버전을 사용했고 재빌드·Editor 테스트는 하지 않았다. 테스트 계정 연결·로그인 성공·무료 구매·영수증 검증·권한 복원은 계속 미완료다.

### 무료 테스트 구매·복원·재시작 검증 완료

사용자가 새 Store 설치의 CustomID를 기존 관리자 생성 테스트 계정에 수동 연결한 후 테스트 로그인이 성공했고 `Store: Ready`가 표시됐다. 앞 절의 `PlayerCreationDisabled` 상태는 이 연결로 해소됐다. 계정 생성 정책은 변경하지 않았다.

Google 구매 화면에서 정확한 격리 패키지와 `No Ads Test`, `테스트 카드, 항상 승인`, `테스트 주문이므로 청구되지 않습니다.`를 확인했다. 표시 가격 KRW 1,000과 무료 테스트 여부를 구분했으며, 사용자가 해당 무료 구매 및 화면의 개인정보처리방침/한국 구매자 약관 동의를 승인한 뒤 구매 1회를 수행했다. Google의 결제 완료 화면을 확인했다.

| 단계 | 직접 관측한 결과 |
|---|---|
| 구매 직후 | 격리 앱 `Store: Ready`, 로컬 저장의 `GooglePlay=ProductNoAds`, 저장 소유자와 테스트 로그인 계정 일치 |
| PlayFab 영수증 처리 | 테스트 플레이어의 레거시 구매 내역 1건이 `성공`, 대상 No Ads SKU의 활성 아이템 1개 |
| Restore purchases | `Store: Ready`로 복귀하고 로컬 No Ads 유지 |
| 격리 앱 강제 종료 후 재실행 | 동일 테스트 계정 로그인 및 Store 연결 성공, `Store: Ready`, 소유자 일치와 로컬 No Ads 유지 |
| 복원·재시작 후 서버 재조회 | 성공 구매 1건, 활성 아이템 1개 유지, 만료/철회/누적 0개 |

이번 실행에서는 복원에 따른 추가 서버 아이템 지급이 관측되지 않았다. PlayFab 구매 화면의 카탈로그 RM 가격 표시는 실제 청구액의 증거로 사용하지 않았다. 무료 여부의 근거는 Google 테스트 결제 화면이다.

구매 확정의 근거에는 한계가 있다. 배포 코드의 `VerifyAndFulfillAsync`는 영수증 검증과 로컬 영구 저장 성공 후 `ConfirmPurchase`를 호출하며, `OnPurchaseConfirmed`의 성공 분기는 `Ready`로 전환한다. 구매 직후 Ready와 영구 저장, 서버 성공 기록은 이 경로의 완료와 부합한다. 그러나 release 로그에서 각 콜백의 직접 추적을 확보하지 못했으므로 Google acknowledgment의 독립적인 상태 조회나 검증→저장→Confirm의 타임스탬프 순서를 직접 관측했다고 주장하지 않는다.

검증 checkout은 `C:/Users/pc_17/.codex/worktrees/7b9b/Tamer`, 문서 작업 시작 기준 커밋은 `65e21de3703cc481e3155a6b8ad817ea5e6ea831`이다. 실행 바이너리는 소스 커밋 `39c2b253e3d6d57e1c3b2e4fc8f2dafe1adac6ef`의 위 AAB 및 Store 전달 APK이며 SHA-256은 앞 표와 같다. Unity `6000.0.81f1`, CLI `1.0.0-beta.8`, IAP `5.4.3`, NDK `27.2.12479018`, Android 16/ARM64/4KB 페이지 환경이다. 코드 변경·재빌드·Editor 테스트 재실행은 없으며 기존 Editor 370/370 결과와 이번 실기기 검증을 구분한다.

앱 삭제/데이터 초기화 후 복원, 다른 기기 복원, 환불·취소·실패 결제 및 실제 16KB 실행은 미검증이다. strict RELRO 3건 실패도 유지한다. 운영 로그인·저장·구매·광고의 검증 결과로 확대하지 않는다. 계정/기기 식별자, CustomID, 주문/트랜잭션 식별자, 영수증 및 원시 로그·화면은 공개 문서에 포함하지 않고 비공개 로컬 증거로 보존한다.
