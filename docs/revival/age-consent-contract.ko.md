...
기존 Editor 테스트에 구간 매핑, 명시적 TFUA true/false, 새 실행의 캐시 우회 방지, SDK 설정 객체의 새 태그/G/구 태그 미지정, 모든 연령의 manager 무호출, 광고 차단 중 Required 옵션 접근과 늦은 보상 차단을 추가했다. 새 네트워크 harness는 만들지 않았다. 검증 결과는 실행 후 별도로 기록한다.
