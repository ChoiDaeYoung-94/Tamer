...
서버 13건, Unity 60건과 Android 일반 플레이어 스크립트 컴파일, 회사폰의 오프라인 두 프로세스 검증을 확인했다. 마지막 파일 정리 보완은 관련 검사 2건만 통과했으며 앞선 검사와 중복 합산하지 않는다. 회사폰 APK에는 마지막 파일 정리 수정이 포함되지 않는다. 검증 소스·APK SHA-256·원본 해시·실패와 미검증 범위는 [삭제 접수 복구 기록](deletion-receipt-recovery.ko.md)에 있다. 운영 endpoint·호스팅·실제 삭제는 미실행이며 기본 비활성이다.

후속 출시 작업은 남아 있다. PGS 설정 읽기에서는 게임 서버 인증 정보 미구성 표시를 확인했으며, 연결된 GCP 프로젝트의 표시된 OAuth 목록에는 Android 클라이언트 2개만 있었고 Web 유형은 없었다. 다른 프로젝트의 존재 여부는 미확인이다. PlayFab WAF가 정상 해소된 뒤 시험 타이틀의 현재 플레이어 3명과 설치된 Google 추가 기능의 OAuth client ID·secret 필드가 비어 있음을 읽기로 확인했다. 시험 패키지는 운영 패키지와 다르며, 시험 패키지와 과거 Play 전달 APK의 서명 근거는 확보했지만, 최신 서명 대조와 목표 PGS 연결은 콘솔 오류로 미확인이다. 기존 native PGS 연결 계정도 단일 조회에서 확인하지 못했다. 과거 계정 수는 현재값으로 사용하지 않는다. 기존 개인정보 처리방침 URL을 보존한 출시용 본문 차이와 초안은 준비 중이며, 광고 지역 동의 메시지 게시·운영 활성화·실제 인증 검증이 완료됐다는 의미가 아니다. 아래 내용은 각 시점의 기존 검증 기록이다.
