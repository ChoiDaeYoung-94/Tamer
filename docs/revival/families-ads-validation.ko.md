...
안전성 보완을 포함한 main `83719f2`를 병합했다. 정책 근거는
...
| 보상 전 닫기, 표시 실패 | 보상 없음, 음악 복구, 재시도/게임 계속 가능 |
| 화면 전환 또는 요청자 파괴 | 기존 요청 무효화; 새 씬의 다른 대상에게 지급하지 않음 |
| 중복 클릭/중복 콜백/오래된 콜백 | 진행 중 두 번째 요청 거절; 종료된 요청이 새 요청을 정리하거나 보상하지 않음 |
...
Android의 Google 직접 공급 광고는 보상을 닫힘 전에 알린다.
[공식 Android 문서](https://developers.google.com/admob/android/rewarded?authuser=19).
그러나 이 저장소의 9.1.1 Editor placeholder는 닫힘 후 같은 호출 묶음에서 보상을 알린다.
...
및 설치 DLL의 IL을 대조했다. 관리자는 한 Update에서 콜백 큐를 비운 뒤 닫힘을 정산한다.
두 순서, 실패와 보상 경합, 종료 후 늦은 보상은 자동 테스트로 검사한다.

이 처리는 임의 mediation 네트워크의 여러 프레임 뒤 보상을 지원한다고 보장하지 않는다.
운영 재활성화 전에 공급자별 순서를 검증해야 한다. 현재 운영 요청과 iOS 기기 요청은 차단되어 있다.
...
./tools/revival/Run-Baseline.ps1 -ProjectPath <이 작업 checkout 절대 경로>
python tools/revival/audit_guids.py
```

2026-09-11 현재 독립 검증: 순수 C# NUnit 49/49, 복원/빌드 사전검사 Python 4/4 통과.
Manager 및 통합 테스트는 설치 Unity 6000.0.81f1/GMA DLL을 참조한 별도 컴파일을 통과했다.
게임 의존성 stub을 사용한 컴파일은 전체 Unity 컴파일·실행을 대신하지 않는다.
Unity EditMode 및 APK 검증은 공유 빌드 순서에 따라 수행 후 이 문서와 PR에 결과를 갱신한다.
