...
2026-09-23 main `410884e`와의 차이는 [Data safety 초안의 통합 대조](data-safety-draft.ko.md)와
[개인정보처리방침 출시 초안](privacy-policy-release-draft.ko.md)에 별도로 기록했다.
아래 9월 11일 감사 결과를 현재 운영 앱이나 제출 가능한 답안으로 읽지 않는다.

## 2026-09-23 Play Console 읽기 대조 — 제출 전 초안

통합 담당이 2026-09-23 운영 Monster Tamer(`com.AeDeong.MonsterTamer`)의 Play Console을
**읽기 전용**으로 확인했다. 아래는 화면에 표시된 선택과 날짜이지, 제공 중인 정확한
AAB/트랙별 데이터 전송을 검증한 결과가 아니다. 이 문서의 정적 코드 기준은 main
`e77864c`이며 Console 답안·정책 URL·대상 연령·광고 설정은 저장하거나 변경하지 않았다.

| Console 화면 | 9월 23일 표시 | 현재 코드·검증과의 대조 |
| --- | --- | --- |
| 앱 콘텐츠 → 데이터 보안 2/5 | 사용자 데이터 수집 또는 공유 `아니요` 선택. 가족 정책 준수 약속 `예` 선택. 마지막 수정 2024-09-30 | PlayFab 계정 식별자·닉네임·진행도·No Ads 권한의 기기 밖 저장 경로가 현재 후보 코드에 있다. [Google의 수집 정의](https://support.google.com/googleplay/android-developer/answer/10787469?hl=en)는 SDK 전송도 포함하므로 `아니요`를 다음 제출 답안으로 그대로 사용할 수 없다. 2024년 선택은 이후 코드·SDK의 검증 근거가 아니다. |
| 앱 콘텐츠 → 개인정보처리방침 | `https://github.com/ChoiDaeYoung-94/Tamer`, 마지막 수정 2024-09-30 | 기존 URL은 보존돼 있으나 [출시 문구 초안](privacy-policy-release-draft.ko.md)은 미게시다. 실제 제공 앱의 처리·삭제·보관을 대조한 뒤 본문과 시행일을 확정해야 한다. |
| 타겟층과 콘텐츠 | 9~12, 13~15, 16~17 선택; 18세 이상 미선택 | 아동 포함 대상이다. 가족 정책 약속 `예`는 [공식 샘플 한 회의 약 8초 닫기 관측](rewarded-close-device-result.ko.md)이나 운영 광고·동의 경로의 적합성 증거가 아니다. 운영 광고 gate는 비활성이다. |
| 앱 대시보드 | production 표시, 일부 변경사항 거부됨 | 거부된 변경과 실제 제공 버전·지역/트랙을 연결해 확인해야 한다. 현재 후보 코드의 보호 조치를 과거 제공 번들에 소급하지 않는다. |

**수정 제안 초안:** 실제 제공 트랙·버전과 다음 AAB별 로그인·PlayFab 저장·구매/복원·광고/UMP
실행 경로를 먼저 목록화한다. 그다음 데이터 유형·목적·필수/선택, SDK/처리자별 수신과
공유 예외, 전송 암호화, 삭제 접수와 보관·파기 근거를 [후보 답안](data-safety-draft.ko.md)에
대조한다. PlayFab에 전송하는 위 계정 귀속 항목은 수집 `예` 방향의 검토 대상이지만,
모든 유형과 공유 여부를 이 관측만으로 확정하지 않는다. [제품 삭제 화면 PR #222](https://github.com/ChoiDaeYoung-94/Tamer/pull/222)는
Draft/Unity 재검증 승인 대기이며, [백업 정리 PR #225](https://github.com/ChoiDaeYoung-94/Tamer/pull/225)의
로컬 격리 검증은 운영 삭제·권한 증거 보관 정책의 완료가 아니다. 실제 제공 경로와
운영 절차의 근거를 확보한 뒤에만 Console 답안 및 정책 게시안을 확정한다.
...
초기 조사에서는 AdMob 관리 화면에 접근하지 못했으나 후속 기존 Chrome 세션에서는 접근에 성공했다.
보상형 단위 1개, 해당 단위의 운영 미디에이션·캠페인 0개, 앱·단위 빈도 제한 없음과
미디에이션 목록의 AdMob 기본 1행을 확인했다. 유럽·미국·IDFA 개인정보 메시지 화면은 새 메시지 생성 안내를 표시했고 게시 메시지 증거는 확인하지 못했다.
설정은 변경하지 않았으며 실제 운영 공급자·소재, 지역·연령별 동의 처리와 통신은 별도 검증 대상이다.
현재 AdMob 관측과 격리 샘플 광고의 실제 동작은 [실기기 광고 기록](families-device-validation.ko.md)에 연결한다.
...
- **PlayFab:** SDK의 `PlayFabDeviceUtil`은 로그인 응답의 `GatherDeviceInfo`/`GatherFocusInfo`와
  ...
  후속 [Console 관측](playfab-console-observation.ko.md)에서 장치 정보·집중 시간 보고, 전체 IP 저장, 새 수집 기능 자동 실행이 켜진 것을 확인했다.
  설정 상태와 실제 추가 장치 정보·화면시간 전송은 구분하며 후자는 아직 확정하지 않는다. 로그인 요청에 들어가는 OS·모델과는 별도 경로다.
