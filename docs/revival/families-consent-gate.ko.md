...
사용자가 확정한 목표는 정상 광고와 기존 NoAds 상품을 함께 복구하고 신규 구매·기존 권한·복원을 보존하는 것이다. 같은 날 네 연령 구간과 응답 거절, 기기 로컬 저장, 데이터가 없을 때 질문하고 설정에서 변경하는 흐름이 승인되어 구현됐다. 지역별 SDK 처리는 아래 검토안으로 남으며 광고 차단은 검증 중 안전장치다.

### 중립적 연령 화면

- 제목: **연령대 확인**
- 설명: **연령에 맞는 광고 및 개인정보 설정을 적용하기 위해 연령대를 선택해 주세요. 생년월일은 저장하지 않습니다.**
- 동일 크기·색상·강조의 선택지: **13세 미만 / 13~15세 / 16~17세 / 18세 이상 / 응답하지 않음**. 기본 선택 없음. 보상·광고량·혜택 차이를 선택지에 표시하지 않는다. 18세 이상 선택지는 실제 이용자 분류용이며 Console 대상 연령 확대가 아니다.
- 광고 및 UMP 처리 전에 표시한다. 건너뛰어도 게임·NoAds 구매 권한·복원·무광고 보상을 제한하지 않는다. 건너뜀은 연령 미상이며 성인으로 추정하지 않는다.
- 승인된 저장 방식: `Tamer.Privacy.AgeChoice` PlayerPrefs에 버전 1과 구간 토큰만 저장한다. 생년월일·정확 나이·계정 연결·서버 전송·분석 이벤트를 추가하지 않는다. 데이터 누락·손상·지원하지 않는 버전은 Unknown으로 재질문한다. 응답 거절도 유효하게 저장하여 실행마다 강제 재질문하지 않는다. 저장 오류는 화면에 알리고 재선택할 수 있게 하며 광고를 차단한다.
...
| 입력 | 광고 처리 제안 | UMP 제안 |
|---|---|---|
| 연령 미상 / 응답하지 않음 | 연령 선택을 마칠 때까지 광고 초기화·요청 0. 기존 NoAds 보상·신규 구매·복원은 유지 | UMP 시작 0. 기존 동의 이력은 임의 삭제하지 않음 |
...
화면·구간·로컬 저장은 승인됐으며 남은 검토는 지역별 처리와 메시지 구성이다. 신규 판매 유지 여부와 Console 연령을 다시 묻지 않는다. 메시지 게시/운영 설정 저장은 구체 화면·문구·대상 지역을 제시한 뒤 별도로 승인받으며 이 문서에서는 실행하지 않았다.

### 로컬 저장 구현과 최소 검증 (2026-09-21)

`LocalAgeChoice`는 저장과 연령 상태를 분리하고, `AgeChoicePresenter`는 Main/Game 진입 시 미상 데이터에 질문하며 기존 설정에서 재선택을 제공한다. 변경 시작 시 광고 캐시·동의 callback·종료된 광고의 늦은 보상 세대를 무효화한다. 연령 변경은 UMP 동의 철회로 기록하지 않는다. NoAds 구매·복원 코드는 변경하지 않았으며 기존 무광고 보상 분기를 유지했다.

pin plugin 11.5.0의 `RequestConfiguration.AgeRestrictedTreatment`는 nullable 필드이며 enum은 Unspecified/Child/Teen이다. 광고 TFCD/TFUA는 deprecated이고 UMP `ConsentRequestParameters.TagForUnderAgeOfConsent`는 별도 bool이다. API 존재 확인과 지역별 처리 검증을 구분한다. 현재 `AgeTreatmentPolicy.IsReviewed`는 모든 구간에서 false이므로 표의 제안을 실제 요청에 적용하지 않으며 sample 경로도 UMP/광고 시작을 차단한다. 운영 광고 게이트도 false로 유지한다.

- checkout: `C:\Users\pc_17\.codex\worktrees\e5e5\Tamer`, 기반 커밋 `a9de1b2e5845ba19b690522eff07541e578f1b97`, 브랜치 `codex/local-age-choice`의 이 구현 변경.
- Unity 6000.0.81f1 / CLI 1.0.0-beta.8 / Android 빌드 대상. Android SDK·패키지 pin은 기존 toolchain과 lock을 유지했다.
- 첫 시도는 `AD.SceneManager` 이름 충돌로 컴파일 중단(테스트 미실행). Unity 클래스 alias 수정 후 두 번째 시도 EditMode **68/68 PASS**, UTC 03:17:43–03:17:44.
- 범위: RevivalAgeChoiceTests, RevivalAgeChoiceUITests, RevivalAdManagerTests, RevivalAdEntitlementTests, RevivalDeletionUITests. 저장 왕복·거절 유지·손상/신버전·저장 오류·동일한 UI 선택지·일시정지 복원·늦은 callback 무효화·기존 권한 경로를 확인했다.
- APK를 새로 만들지 않아 이번 APK SHA-256은 없다. 기기 UI 시각 검증, 실제 구매/복원, 지역별 동의/개인정보 옵션 폼, 5초 종료 및 운영 활성화는 미검증이다. 테스트가 생성한 폰트·시간 설정 변경은 복원했으며 원시 로그는 공개하지 않는다.
