...
팝업 닫기나 프로세스 종료는 서버에 접수된 삭제 취소가 아니다. 앱 core는 요청 ID/인증 proof를 디스크에
보존하지 않는다. 새 clientKey로 다시 재인증해 요청하면 서버가 동일 계정의 미완료 요청을 찾아 이어 준다.
확인 대기 중이면 새 challenge를 발급하고, 이미 접수됐으면 같은 처리 상태를 반환한다.
완료된 계정의 재인증/상태 접근은 운영 인증 설계가 필요하며 이 합성 시험과 구분한다.
