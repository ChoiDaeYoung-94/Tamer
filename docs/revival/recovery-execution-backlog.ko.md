...
최종 README는 게임 소개 → 실제 플레이 스크린샷/필요한 제작 이미지 → 기존 플레이 영상 → 검증된 다운로드 안내 → 개발 문서 순으로 구성해 읽기 쉽게 만든다. 현재 광고 등 주요 검증을 방해하지 않고 최종 개편 단계에서 구현한다. 이 항목은 요구 기록이며 개편 완료가 아니다.
...
- 현재 README에는 인라인 게임 이미지가 없다. 로컬 이미지 후보는 `Assets/Art/Images/title.png`, `icon.png`, `map.png`, `newmap.png`이며 현재 단계에서는 위치만 조사했다. 후보의 출처·공개 권한·실제 외형을 확인한 뒤 선택하고 구매 에셋 원본을 추가 공개하지 않는다. 비공개 검증 화면도 그대로 게시하지 않는다.
- 실제 게임의 전투·동료·마을을 보여 주는 공개 가능한 플레이 스크린샷을 준비한다. 필요하면 imagegen 스킬/도구로 실제 외형에 맞는 소개 이미지를 제작하고 제작 이미지임을 표시한다. 생성 이미지를 실제 플레이 화면처럼 안내하지 않는다.
