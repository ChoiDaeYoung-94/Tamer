...
아래 의존성은 소스의 lexical 참조이며 전체 호출 그래프가 아니다. 감사 기준은 `fd879eccb11c50409fd204a3bb248c75a11c8883`이며, 통합 검증 소스 `300d427e4c557fd3cc4cda347b255a22a9a53bc7` 이후 자체 C# 변경은 없다. 문서 감사 때문에 Unity를 다시 실행하지 않았다.
...
| `Assets/Scripts/Advertising/AdRequestPolicy.cs` | 테스트 광고 요청·공식 sample ID 제한 |  /  | 통합: batch/production 차단 및 명시적 Android test 조건 유지 | RevivalAdRequestPolicyTests; 통합317; UMP 후속 별도 |
...
| `Assets/Scripts/Cameras/CameraManage.cs` | 영속 Cinemachine 타깃 |  / Player | 통합: 자신의 singleton만 종료 시 해제, 이전 객체가 새 owner를 지우지 않음 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
| `Assets/Scripts/Creatures/BuffingMan.cs` | 광고/No Ads 버프 진입 UI | DataM, GoogleAdMobM / Player, PlayerUICanvas | 통합: 기존 광고 세션 계약 유지, 자신의 singleton 종료 처리 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/Creatures/Monster.cs` | AI·탐지·전투·포획·풀 복귀 | DataM, PoolM / MonsterGenerator, Player | 통합: 사망/비활성/미배치 agent Update 차단, 탐지 재시작 시 이전 CTS 취소·해제 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
| `Assets/Scripts/Creatures/Player.cs` | 플레이어 전투·이동·보유 데이터 | DataM, EquipmentM, GameM, Instance, PoolM, SoundM, UpdateM / BuffingMan, CameraManage, JoyStick, Managers, PlayerUICanvas | 통합: publisher 소유 구독 정리, 저장 no-op 원문 보존, 조회 대상의 Gold 반환 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
| `Assets/Scripts/Creatures/ShopMan.cs` | 상점 구매·장착·목록 | EquipmentM, IAPM, PopupM, SoundM / Player | 통합: 구매 확인 1회 소비, 금액/잔액/보유 재검사, singleton 소유권 정리 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/Game/MonsterGenerator.cs` | 생성 수·군집·생성 루프 | PoolM / Player | 통합: 반복 Init/Disable 멱등, CTS 정리와 군집 목록 snapshot 회수 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
| `Assets/Scripts/Game/Portal.cs` | 이동·치유 트리거 | PopupM / Player | 통합: 자신의 singleton만 종료 시 해제; 이동/치유 규칙 유지 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/Main/IAPItem.cs` | No Ads 구매 항목 표시 | DataM / ShopMan | 통합: 등록한 ShopMan에서 해제; 기존 No Ads 저장 키 계약 유지 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/Main/Item.cs` | 상점 아이템 표시·선택 | DataM, EquipmentM / Player, ShopMan | 통합: 등록한 ShopMan에서 해제, 잠금 상태를 양방향 갱신 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/Managers/GoogleAdMobManager.cs` | 광고 SDK·callback queue·세션·오디오 소유 | DataM, Instance, SoundM / BuffingMan, Managers, Player, PlayerUICanvas | 통합: 전체 소스 확인. 지연 callback generation, 30초 load, 종료 정리, 원래 보상 수령자 유지 | 통합317 광고 회귀 및 이전 test-device 근거; UMP 변경은 별도 진행 중 |
...
| `Assets/Scripts/MiniMap/FogOfWar/FogOfWarRenderer.cs` | 안개 cell texture/blur 렌더링 |  /  | 통합: Texture2D/Color 버퍼 재사용, 소유 material/texture/RT 정리, 임시 RT finally 반환 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/MiniMap/MiniMap.cs` | 지도 아이콘·드래그·일시정지 | DataM, Instance, UpdateM / JoyStick, Managers, Player, PlayerUICanvas | 통합: 캡처한 publisher에서 해제, 저장한 timeScale을 멱등 복구. 중첩된 0 pause owner의 전역 조정은 지원하지 않음 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
| `Assets/Scripts/MiniMap/MiniMapCanvas.cs` | 지도 닫기 중계 |  / MiniMap | 통합: OnEnable에서 캡처한 MiniMap owner만 OnDisable에서 닫음 | PR125; 통합 317/317, gameplay-followup-validation.json. 실제 Main/Game 왕복은 미검증 |
...
| `Assets/Scripts/RevivalAdHarness.cs` | 격리 sample/control 광고 수동 실험 UI | Instance / Managers | 통합: test-only owner/scene/manager 파괴 실험과 event 추적 유지; 운영 진입 경로 아님 | 소스 감사 및 기존 기기 실험; UMP 후속 artifact 검증 진행 중 |
...
98개 파일의 소스 감사 판단을 모두 연결했다. 확정한 gameplay 결함은 PR122/125에서 수정했고 전체 317개 회귀가 통과했다. 소스 감사 완료는 실제 Main/Game 왕복, 모든 prefab/화면 크기/시야 조합, 운영 인증·저장·구매, 정책 적합성 완료를 뜻하지 않는다. 임의 CellSize·빈 map prefab·중첩 pause 등 구성 일반화는 현재 계약과 검증 한계를 각 행에 기록했다. 진행 중인 UMP 신규 파일은 병합 후 inventory와 감사표에 추가한다.
