...
범위: 추적 중인 Assets/Scripts, Assets/Tests, Assets/GPGSIds.cs의 116개 파일. SDK 디렉터리는 별도 공급망 감사 범위다. Scripts/MiniMap/FogOfWar는 출처만으로 제외하지 않고 포함한다. 신규 파일은 통합 시 inventory를 다시 생성해 추가한다.
...
| --- | --- | --- | --- | --- |
| `Assets/Scripts/Privacy/SyntheticLoopbackDeletionGateway.cs` | Editor/dev 합성 loopback HTTP 계약 |  /  | 서비스/통합: 고정 합성 신원·config 확인, proxy/redirect 차단, 응답 제한·상태 검증; 호출자 Dispose | .NET9 실제 Python 소켓13/13, Unity355/355 컴파일·회귀; deletion-loopback-http-validation.json |
...
116개 파일의 소스 감사 판단을 연결했다. 최신 Editor355개 회귀·.NET9 HTTP 계약13개와 이전 오프라인 APK의 실제 씬 왕복 2회를 구분한다. 원래 씬을 실행했지만 합성 계정·메모리 서버로 격리했으며 운영 인증/진행도 쓰기/구매/정책/16KB 완료를 뜻하지 않는다. 최신 근거는 [격리 gameplay 검증](gameplay-harness.ko.md)과 [UMP 검증](families-consent-gate.ko.md)에 있다.

## PlayFab 테스트 검증기 추가

inventory 기준 `e0f87800b4d7c94b7c28dc884b9b2f98431a24a9`에서 신규 3개를 포함해 총 116개이며 meta 누락은 없다. 동일 소스 Editor361/361/0skip(4.6079884초)를 통과했다. 이전 113개·355개 결과는 위에 이력으로 유지한다.

| 파일 | 책임 | 검토 결과 | 검증 |
| --- | --- | --- | --- |
| `Assets/Scripts/PlayFabIapReceiptVerifier.cs` | 별도 타이틀 instance SDK 연결 | SDK·통합: static 설정 불변, package/catalog/item 검사, 캡처 티켓 inventory; 실제 Google add-on 미설정 | 컴파일361; 원격 구매 미검증 |
| `Assets/Scripts/Purchasing/PlayFabNoAdsVerification.cs` | 중복 영수증 권한 판정 | SDK·통합: AlreadyUsed만으로 지급 불가, 동일 세션 inventory 필수, 취소 후 차단 | 신규 합성 회귀6 포함 |
