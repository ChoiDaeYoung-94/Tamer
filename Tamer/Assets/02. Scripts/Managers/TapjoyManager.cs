...
public class TapjoyManager : MonoBehaviour
{
    ...
    void TapjoyInit()
    {
        ...
        if (!Tapjoy.IsConnected)
            Tapjoy.Connect(_str_androidKey);
#elif UNITY_IOS
		if (!Tapjoy.IsConnected)
            Tapjoy.Connect(_str_iosKey);
#endif

        Tapjoy.SetUserID(Server.Player.ID);

        _placement = TJPlacement.CreatePlacement(_str_placement);
    ...
    {
        Debug.Log("TapJoy Connect Failed -> reconnct");
    ...
    {
        Debug.Log("HandleGetCurrencyBalanceResponse: currencyName: " + currencyName + ", balance: " + balance);
    ...
    {
        Debug.Log("HandleGetCurrencyBalanceResponseFailure: " + error);
    }
    ...
    {
        Debug.Log("HandleAwardCurrencySucceeded: currencyName: " + currencyName + ", balance: " + balance);
    }

    void HandleAwardCurrencyResponseFailure(string error)
    {
        Debug.Log("HandleAwardCurrencyResponseFailure: " + error);
    }

    void HandleSpendCurrencyResponse(string currencyName, int balance)
    {
        Debug.Log("HandleSpendCurrencyResponse: currencyName: " + currencyName + ", balance: " + balance);

        Debug.Log($"cur balance - {balance}");
        if (balance == 0)
        {
    ...
    {
        Debug.Log("HandleSpendCurrencyResponseFailure: " + error);
    }

    void HandlePlacementRequestSuccess(TJPlacement placement)
    {
        Debug.Log("HandlePlacementRequestSuccess: placementName: " + placement.GetName());
    }

    void HandlePlacementRequestFailure(TJPlacement placement, string error)
    {
        Debug.Log("HandlePlacementRequestFailure: " + error);
    ...
    {
        Debug.Log("HandlePlacementContentReadySuccess: placementName: " + placement.GetName());
    }
    ...
    {
        Debug.Log("HandlePlacementContentShow: placementName: " + placement.GetName());
        _placement.RequestContent();
    }
