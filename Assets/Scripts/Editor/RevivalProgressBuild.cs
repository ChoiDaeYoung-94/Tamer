...
public static class RevivalProgressBuild
{
    ...
    public static string ProgressManifest(string original)
    {
        ...
        foreach (string permission in new[] { "com.android.vending.BILLING", "com.google.android.gms.permission.AD_ID" })
        {
    ...
    public static void BuildAndroid()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Manifest, ProgressManifest(File.ReadAllText(Manifest)));
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Android, RevivalProgressHarness.ApplicationId);
            ...
            var report = BuildPipeline.BuildPlayer(new BuildPlayerOptions
            {
                ...
                locationPathName = "Build/revival/Tamer-progress.apk",
                options = BuildOptions.Development | BuildOptions.CompressWithLz4,
                extraScriptingDefines = new[] { "TAMER_REVIVAL_SMOKE", "TAMER_PROGRESS_HARNESS" }
            });
            if (report.summary.result != BuildResult.Succeeded) throw new BuildFailedException("Progress build failed.");
            Debug.Log("PROGRESS_BUILD_OK dedicated=true noPurchasing=true");
            code = 0;
        }
        ...
        finally
        {
            ...
            if (originals.Any(entry => !File.ReadAllBytes(entry.Key).SequenceEqual(entry.Value)))
                throw new BuildFailedException("Progress build settings restoration failed.");
        }
        if (Application.isBatchMode) EditorApplication.Exit(code);
        else if (code != 0) throw new BuildFailedException("Progress build failed.");
    }
