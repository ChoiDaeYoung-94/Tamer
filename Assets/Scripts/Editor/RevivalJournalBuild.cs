...
public static class RevivalJournalBuild
{
    ...
    public static string JournalManifest(string original)
    {
    ...
    public static void BuildAndroid()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Manifest, JournalManifest(File.ReadAllText(Manifest)));
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Android, RevivalJournalHarness.ApplicationId);
            ...
            var report = BuildPipeline.BuildPlayer(new BuildPlayerOptions
            {
                ...
                locationPathName = "Build/revival/Tamer-journal.apk",
                options = BuildOptions.Development | BuildOptions.CompressWithLz4,
                extraScriptingDefines = new[] { "TAMER_REVIVAL_SMOKE", "TAMER_JOURNAL_HARNESS" }
            });
            if (report.summary.result != BuildResult.Succeeded) throw new BuildFailedException("Journal build failed.");
            Debug.Log("JOURNAL_BUILD_OK dedicated=true noPurchasing=true");
            code = 0;
        }
        ...
        finally
        {
            ...
            if (originals.Any(entry => !File.ReadAllBytes(entry.Key).SequenceEqual(entry.Value)))
                throw new BuildFailedException("Journal build settings restoration failed.");
        }
        if (Application.isBatchMode) EditorApplication.Exit(code);
        else if (code != 0) throw new BuildFailedException("Journal build failed.");
    }
